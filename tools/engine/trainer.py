...
class Trainer(object):
    def __init__(self, cfg, mode='train'):
        ...
        self.writer = None
        if self.cfg['Global']['use_tensorboard'] and 'train' in mode:
        ...
        self.all_ema = self.cfg['Global'].get('all_ema', False)
        self.use_ema = self.cfg['Global'].get('use_ema', False)

        self.set_random_seed(self.cfg['Global'].get('seed', 48))

        # 构建数据加载器，去除DistributedSampler
        self.train_dataloader = None
        if 'train' in mode:
            cfg.save(os.path.join(self.cfg['Global']['output_dir'], 'config.yml'), self.cfg)
            self.train_dataloader = build_dataloader(self.cfg, 'Train', self.logger)  # 不使用分布式采样
        ...
        if self.cfg['Global']['distributed'] and torch.cuda.device_count() > 1:
            ema_model = build_model(self.cfg['Architecture'])
            self.ema_model = torch.nn.DataParallel(ema_model.to(self.device))
            self.ema_model.eval()
    ...
    def set_device(self, device_type):
        self.device = torch.device(f'cuda:{self.local_rank}' if device_type == 'gpu' and torch.cuda.is_available() else 'cpu')
